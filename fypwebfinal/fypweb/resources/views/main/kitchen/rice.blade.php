                    ...
                    <div class="col-xs-4 product-men" style="margin-top: 25px;">
                        <div class="men-pro-item simpleCart_shelfItem">
                            <div class="men-thumb-item">
                                <img src="{{UtilsHelper::resolveImage($rice->image)}}" alt="" style="height: 150px;">
                                <div class="men-cart-pro">
                                    <div class="inner-men-cart-pro">
                                        <a href="/rice/{{$rice->id}}" class="link-product-add-cart">Quick View</a>
                                    </div>
                                </div>
                                @if ($rice->productStock<=0)  
                            ...
                            <div class="item-info-product ">
                                ...
                                @if($rice->discounts)
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$rice->price-$rice->discounts}} </span>
                                        <del>{{$rice->price}}</del>
                                    </div>
                                    @else
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$rice->price}} </span>
                                ...
                                <form action="/Cart/add"method="post">
                                    ...
                                    <input type="hidden" name="category" value="rice" />
                                            <input type="hidden" name="pdt_id" value="{{$rice->id}}" />
                                            <input type="hidden" name="name" value="{{$rice->name}}" />
                                            <input type="hidden" name="price" value="{{$rice->price}}" />
                                            <input type="hidden" name="quantity" value="1" />

                                            <button type="submit" class="buttonsubmit" style="margin-top: -10px; width:180px" 
                                            @if ($rice->productStock<=0) disabled  @endif>
                                                Add to cart
                                            </button>
                                            @auth
                                            <a href="javascript:void(0)" class="link-product-add-cart" onclick="addwishlist({{$rice->id}})" style="width: 50px; float:right; margin-top:-9px;">
                                                @if($rice->wishlist)
                                                <i class="fa fa-heart icon-{{$rice->id}}"></i>
                                                @else
                                                <i class="fa fa-heart-o icon-{{$rice->id}}"></i>
                                                @endif
                                            </a>
                                            @endauth
