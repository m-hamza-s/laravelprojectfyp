                    ...
                    <div class="col-xs-4 product-men" style="margin-top: 25px;">
                        <div class="men-pro-item simpleCart_shelfItem">
                            <div class="men-thumb-item">
                                <img src="{{UtilsHelper::resolveImage($sweet->image)}}" alt="" style="height: 150px;">
                                <div class="men-cart-pro">
                                    <div class="inner-men-cart-pro">
                                         <a href="/sweets/{{$sweet->id}}" class="link-product-add-cart">Quick View</a> 
                                    </div>
                                </div>
                                @if ($sweet->productStock<=0)  
                            ...
                            <div class="item-info-product ">
                                <h4>
                                    <a href="single.html">{{$sweet->name}}</a>
                                </h4>
                                @if($sweet->discounts)
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$sweet->price-$sweet->discounts}} </span>
                                        <del>{{$sweet->price}}</del>
                                    </div>
                                    @else
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$sweet->price}} </span>
                                ...
                                <form action="/Cart/add"method="post">
                                    ...
                                    <input type="hidden" name="category" value="sweet" />
                                            <input type="hidden" name="pdt_id" value="{{$sweet->id}}" />
                                            <input type="hidden" name="name" value="{{$sweet->name}}" />
                                            <input type="hidden" name="price" value="{{$sweet->price}}" />
                                            <input type="hidden" name="quantity" value="1" />

                                            <button type="submit" class="buttonsubmit" style="margin-top: -10px; width:180px" 
                                            @if ($sweet->productStock<=0) disabled  @endif>
                                                Add to cart
                                            </button>
                                            @auth
                                            <a href="javascript:void(0)" class="link-product-add-cart" onclick="addwishlist({{$sweet->id}})" style="width: 50px; float:right; margin-top:-9px;">
                                                @if($sweet->wishlist)
                                                <i class="fa fa-heart icon-{{$sweet->id}}"></i>
                                                @else
                                                <i class="fa fa-heart-o icon-{{$sweet->id}}"></i>
                                                @endif
                                            </a>
                                            @endauth
                                    </form>
