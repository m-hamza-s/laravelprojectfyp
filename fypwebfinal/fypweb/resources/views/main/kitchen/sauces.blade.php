                    ...
                    <div class="col-xs-4 product-men" style="margin-top: 25px;">
                        <div class="men-pro-item simpleCart_shelfItem">
                            <div class="men-thumb-item">
                                <img src="{{UtilsHelper::resolveImage($sauce->image)}}" alt="" style="height: 150px;">
                                <div class="men-cart-pro">
                                    <div class="inner-men-cart-pro">
                                        <a href="/sauces/{{$sauce->id}}" class="link-product-add-cart">Quick View</a>
                                    </div>
                                </div>
                                @if ($sauce->productStock<=0)  
                            ...
                            <div class="item-info-product ">
                                ...
                                @if($sauce->discounts)
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$sauce->price-$sauce->discounts}} </span>
                                        <del>{{$sauce->price}}</del>
                                    </div>
                                    @else
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$sauce->price}} </span>
                                    
                                    </div>
                                    @endif
                                <form action="/Cart/add"method="post">
                                    {{ csrf_field() }}
                                    <input type="hidden" name="category" value="sauce" />
                                            <input type="hidden" name="pdt_id" value="{{$sauce->id}}" />
                                            <input type="hidden" name="name" value="{{$sauce->name}}" />
                                            <input type="hidden" name="price" value="{{$sauce->price}}" />
                                            <input type="hidden" name="quantity" value="1" />

                                            <button type="submit" class="buttonsubmit" style="margin-top: -10px; width:180px" 
                                            @if ($sauce->productStock<=0) disabled  @endif>
                                                Add to cart
                                            </button>
                                            @auth
                                            <a href="javascript:void(0)" class="link-product-add-cart" onclick="addwishlist({{$sauce->id}})" style="width: 50px; float:right; margin-top:-9px;">
                                                @if($sauce->wishlist)
                                                <i class="fa fa-heart icon-{{$sauce->id}}"></i>
                                                @else
                                                <i class="fa fa-heart-o icon-{{$sauce->id}}"></i>
                                                @endif
                                            </a>
                                            @endauth
                                    </form>
