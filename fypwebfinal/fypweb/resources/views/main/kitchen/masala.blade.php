                    ...
                    <div class="col-xs-4 product-men" style="margin-top: 25px;">
                        <div class="men-pro-item simpleCart_shelfItem">
                            <div class="men-thumb-item">
                                <img src="{{UtilsHelper::resolveImage($spice->image)}}" alt="" style="height: 150px;">
                                <div class="men-cart-pro">
                                    <div class="inner-men-cart-pro">
                                        <a href="/masalas/{{$spice->id}}" class="link-product-add-cart">Quick View</a>
                                    </div>
                                </div>
                                @if ($spice->productStock<=0)  
                            ...
                            <div class="item-info-product ">
                                ...
                                @if($spice->discounts)
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$spice->price-$spice->discounts}} </span>
                                        <del>{{$spice->price}}</del>
                                    </div>
                                    @else
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$spice->price}} </span>
                                    
                                    </div>
                                    @endif
                                <form action="/Cart/add"method="post">
                                    {{ csrf_field() }}
                                    <input type="hidden" name="category" value="masala" />
                                            <input type="hidden" name="pdt_id" value="{{$spice->id}}" />
                                            <input type="hidden" name="name" value="{{$spice->name}}" />
                                            ...
                                            @if ($spice->productStock<=0) disabled  @endif >
                                                Add to cart
                                            </button>
                                            @auth
                                            <a href="javascript:void(0)" class="link-product-add-cart" onclick="addwishlist({{$spice->id}})" style="width: 50px; float:right; margin-top:-9px;">
                                                @if($spice->wishlist)
                                                <i class="fa fa-heart icon-{{$spice->id}}"></i>
                                                @else
                                                <i class="fa fa-heart-o icon-{{$spice->id}}"></i>
                                                @endif
                                            </a>
                                            @endauth
