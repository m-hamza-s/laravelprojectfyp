                    ...
                    <div class="col-xs-4 product-men"style="margin-top: 25px;">
                        <div class="men-pro-item simpleCart_shelfItem">
                            <div class="men-thumb-item">
                                <img src="{{UtilsHelper::resolveImage($coffee->image)}}" alt="" style="height: 150px;">
                                <div class="men-cart-pro">
                                    <div class="inner-men-cart-pro">
                                        <a href="/coffee/{{$coffee->id}}" class="link-product-add-cart">Quick View</a>
                                    </div>
                                </div>
                                @if ($coffee->productStock<=0)  
                            ...
                            <div class="item-info-product ">
                                ...
                                @if($coffee->discounts)
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$coffee->price-$coffee->discounts}} </span>
                                        <del>{{$coffee->price}}</del>
                                    </div>
                                    @else
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$coffee->price}} </span>
                                ...
                                <form action="/Cart/add"method="post">
                                    ...
                                    <input type="hidden" name="category" value="coffee" />
                                            <input type="hidden" name="pdt_id" value="{{$coffee->id}}" />
                                            <input type="hidden" name="name" value="{{$coffee->name}}" />
                                            <input type="hidden" name="price" value="{{$coffee->price}}" />
                                            <input type="hidden" name="quantity" value="1" />

                                            <button type="submit" class="buttonsubmit" style="margin-top: -10px; width:180px" @if ($coffee->productStock<=0) disabled  @endif>
                                                Add to cart
                                            </button>
                                            
                                            @auth
                                            <a href="javascript:void(0)"class="link-product-add-cart" onclick="addwishlist({{$coffee->id}})" style="width: 50px; float:right; margin-top:-9px;"  >
                                                @if($coffee->wishlist)
                                                <i class="fa fa-heart icon-{{$coffee->id}}"></i>
                                                @else
                                                <i class="fa fa-heart-o icon-{{$coffee->id}}"></i>
                                                @endif
                                            </a>
                                            @endauth
