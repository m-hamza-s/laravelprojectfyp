                    ...
                    <div class="col-xs-4 product-men" style="margin-top: 25px;">
                        <div class="men-pro-item simpleCart_shelfItem">
                            <div class="men-thumb-item">
                                <img src="{{UtilsHelper::resolveImage($snack->image)}}" alt="" style="height: 150px;">
                                <div class="men-cart-pro">
                                    <div class="inner-men-cart-pro">
                                        <a href="/snack/{{$snack->id}}" class="link-product-add-cart">Quick View</a>
                                    </div>
                                </div>
                                @if ($snack->productStock<=0)  
                            ...
                            <div class="item-info-product ">
                                ...
                                <div class="info-product-price">
                                    <span class="item_price">PKR. {{$snack->price-$snack->discounts}} </span>
                                    <del>{{$snack->price}}</del>
                                </div>
                                @else
                                <div class="info-product-price">
                                    <span class="item_price">PKR. {{$snack->price}} </span>
                                ...
                                <form action="/Cart/add"method="post">
                                    ...
                                    <input type="hidden" name="category" value="snack" />
                                            <input type="hidden" name="pdt_id" value="{{$snack->id}}" />
                                            ...
                                            @if ($snack->productStock<=0) disabled  @endif
                                            >
                                                Add to cart
                                            </button>
                                            @auth
                                            <a href="javascript:void(0)" class="link-product-add-cart" onclick="addwishlist({{$snack->id}})" style="width: 50px; float:right; margin-top:-9px;">
                                                @if($snack->wishlist)
                                                <i class="fa fa-heart icon-{{$snack->id}}"></i>
                                                @else
                                                <i class="fa fa-heart-o icon-{{$snack->id}}"></i>
                                                @endif
                                            </a>
                                            @endauth
