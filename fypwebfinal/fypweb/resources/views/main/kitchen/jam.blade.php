                    ...
                    <div class="col-xs-4 product-men" style="margin-top: 25px;">
                        <div class="men-pro-item simpleCart_shelfItem">
                            <div class="men-thumb-item">
                                <img src="{{UtilsHelper::resolveImage($jam->image)}}" alt="" style="height: 150px;">
                                <div class="men-cart-pro">
                                    <div class="inner-men-cart-pro">
                                        <a href="/jams/{{$jam->id}}" class="link-product-add-cart">Quick View</a>
                                    </div>
                                </div>
                                @if ($jam->productStock<=0)  
                            ...
                            <div class="item-info-product ">
                                ...
                                @if($jam->discounts)
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$jam->price-$jam->discounts}} </span>
                                        <del>{{$jam->price}}</del>
                                    </div>
                                    @else
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$jam->price}} </span>
                                    
                                    </div>
                                    @endif
                                <form action="/Cart/add"method="post">
                                    {{ csrf_field() }}
                                            <input type="hidden" name="category" value="jam" />
                                            <input type="hidden" name="pdt_id" value="{{$jam->id}}" />
                                            <input type="hidden" name="name" value="{{$jam->name}}" />
                                            <input type="hidden" name="price" value="{{$jam->price}}" />
                                            <input type="hidden" name="quantity" value="1" />

                                            <button type="submit" class="buttonsubmit" style="margin-top: -10px; width:180px" 
                                            @if ($jam->productStock<=0) disabled  @endif>
                                                Add to cart
                                            </button>
                                            @auth
                                            <a href="javascript:void(0)" class="link-product-add-cart" onclick="addwishlist({{$jam->id}})" style="width: 50px; float:right; margin-top:-9px;" >
                                                @if($jam->wishlist)
                                                <i class="fa fa-heart icon-{{$jam->id}}"></i>
                                                @else
                                                <i class="fa fa-heart-o icon-{{$jam->id}}"></i>
                                                @endif
                                            </a>
                                            @endauth
