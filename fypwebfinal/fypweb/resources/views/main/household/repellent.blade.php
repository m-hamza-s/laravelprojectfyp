                    ...
                    <div class="col-xs-4 product-men" style="margin-top: 25px;">
                        ...
                        <div class="men-pro-item simpleCart_shelfItem">
                            <div class="men-thumb-item">
                                <img src="{{UtilsHelper::resolveImage($repel->image)}}" alt="" style="height: 150px;">
                                <div class="men-cart-pro">
                                    <div class="inner-men-cart-pro">
                                        <a href="/repellent/{{$repel->id}}" class="link-product-add-cart">Quick View</a>
                                    </div>
                                </div>
                                @if ($repel->productStock<=0)  
                            ...
                            <div class="item-info-product ">
                                ...
                                @if($repel->discounts)
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$repel->price-$repel->discounts}} </span>
                                        <del>{{$repel->price}}</del>
                                    </div>
                                    @else
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$repel->price}} </span>
                                    
                                    </div>
                                    @endif
                                <form action="/Cart/add"method="post">
                                    {{ csrf_field() }}
                                    <input type="hidden" name="category" value="repellent" />
                                            <input type="hidden" name="pdt_id" value="{{$repel->id}}" />
                                            <input type="hidden" name="name" value="{{$repel->name}}" />
                                            <input type="hidden" name="price" value="{{$repel->price}}" />
                                            <input type="hidden" name="quantity" value="1" />

                                            <button type="submit" class="buttonsubmit" style="margin-top: -10px; width:180px"
                                            @if ($repel->productStock<=0) disabled  @endif>
                                                Add to cart
                                            </button>
                                            @auth
                                            <a href="javascript:void(0)" class="link-product-add-cart" onclick="addwishlist({{$repel->id}})" style="width: 50px; float:right; margin-top:-9px;">
                                                @if($repel->wishlist)
                                                <i class="fa fa-heart icon-{{$repel->id}}"></i>
                                                @else
                                                <i class="fa fa-heart-o icon-{{$repel->id}}"></i>
                                                @endif
                                            </a>
                                            @endauth
                                    </form>
