                    ...
                    <div class="col-xs-4 product-men" style="margin-top: 25px;">
                        ...
                        <div class="men-pro-item simpleCart_shelfItem">
                            <div class="men-thumb-item">
                                <img src="{{UtilsHelper::resolveImage($detergent->image)}}" alt="" style="height: 150px;">
                                <div class="men-cart-pro">
                                    <div class="inner-men-cart-pro">
                                        <a href="/detergent/{{$detergent->id}}" class="link-product-add-cart">Quick View</a>
                                    </div>
                                </div>
                                @if ($detergent->productStock<=0)  
                            ...
                            <div class="item-info-product ">
                                <h4>
                                    <a href="">{{$detergent->name}} </a>
                                </h4>
                                @if($detergent->discounts)
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$detergent->price-$detergent->discounts}} </span>
                                        <del>{{$detergent->price}}</del>
                                    </div>
                                    @else
                                    <div class="info-product-price">
                                        <span class="item_price">PKR. {{$detergent->price}} </span>
                                    
                                    </div>
                                    @endif
                                <form action="/Cart/add"method="post">
                                    {{ csrf_field() }}
                                    <input type="hidden" name="category" value="detergent" />
                                            <input type="hidden" name="pdt_id" value="{{$detergent->id}}" />
                                            <input type="hidden" name="name" value="{{$detergent->name}}" />
                                            <input type="hidden" name="price" value="{{$detergent->price}}" />
                                            <input type="hidden" name="quantity" value="1" />

                                            <button type="submit" class="buttonsubmit" style="margin-top: -10px; width:180px" 
                                            @if ($detergent->productStock<=0) disabled  @endif>
                                                Add to cart
                                            </button>
                                            @auth
                                            <a href="javascript:void(0)" class="link-product-add-cart" onclick="addwishlist({{$detergent->id}})" style="width: 50px; float:right; margin-top:-9px;">
                                                @if($detergent->wishlist)
                                                <i class="fa fa-heart icon-{{$detergent->id}}"></i>
                                                @else
                                                <i class="fa fa-heart-o icon-{{$detergent->id}}"></i>
                                                @endif
                                            </a>
                                            @endauth
